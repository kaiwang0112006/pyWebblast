...
# def blastnHome(request):
#     blastdbconf = request.registry['nucleotidedb']
#     
#     return {'dbname':blastdbconf.keys()}

@view_config(route_name='blastnrun',renderer='json')
def blastnrun(request):
    ...
    outname = timestr + '_' + parameters_data['jobtitle'] + '_blastn.out'
    out11 = os.path.join(temppath, outname)
    #setup blast cmd
    blastcmd = '%sblastn -query %s -db %s -outfmt 11 -out %s' % (ncbibase, queryfa, db, out11)
