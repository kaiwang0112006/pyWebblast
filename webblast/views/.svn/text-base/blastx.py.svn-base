...
@view_config(route_name='blastxrun',renderer='json')
def blastxrun(request):
    ...
    outname = timestr + '_' + parameters_data['jobtitle'] + '_blastx.out'
    out11 = os.path.join(temppath, outname)
    #setup blast cmd
    blastcmd = '%sblastx -query %s -db %s -outfmt 11 -out %s' % (ncbibase, queryfa, db, out11)
    ...
    if p.returncode and stderr !='':
        ...
        outfmtfileReturn = blastformat(timestr,parameters_data,temppath,staticpath,ncbibase,'blastx')
